<?php

ini_set('memory_limit','2048M');

$file = $argv[1] ?? "input";
$test = $file == "test";

require_once(__DIR__."/../inputReader.php");
require_once(__DIR__."/../Util.php");

const NONE = 0;
const WALL = 1;
const SPACE = 2;

const EAST = 0;
const SOUTH = 1;
const WEST = 2;
const NORTH = 3;

$ir = (new InputReader(__DIR__ . DIRECTORY_SEPARATOR . $file));
$grid = $ir->grid(["#" => WALL, "." => SPACE, " " => NONE]);
$grid = array_map("array_filter", $grid);

$path = ($test) ?
"10R5L5R10L4R5L5" :
"";

$chars = str_split($path);
$count = "";

$dirs = [EAST, SOUTH, WEST, NORTH];
$dir = 0; // starting at east ($dir[0])

$movements = [
    [0, 1, EAST], #
    [1, 0, SOUTH], #
    [0, -1, WEST], #
    [-1, 0, NORTH], #
];
$movementChars = [">", "v", "<", "^"];

$planeWidth = min(array_map("count", $grid));
for ($x = 0; $x < count($grid); $x++) {
    $c = 0;
    for ($y = 0; $y < count($grid); $y++) {
        $c += (isset($grid[$y][$x])) ? 1 : 0;
    }
    $planeWidth = min($planeWidth, $c);
}
$ys = count($grid)/$planeWidth;
$xs = 12/$ys;
$planes = [];
for ($y = 0; $y < $ys; $y++) {
    for ($x = 0; $x < $xs; $x++) {
        if (isset($grid[$y*$planeWidth][$x*$planeWidth])) {
            $planes[$y][$x] = [];
        }
    }
}

foreach ($planes as $y => $row) {
    foreach ($row as $x => $setting) {
        $found = 0;
        $d = 1;
        echo "$y,$x\n";
        $q = [ [$y, $x, 0, false] ];
        while (count($planes[$y][$x]) < 4) {
            list($tmpY, $tmpX, $d, $origMoveDir) = array_shift($q);
            echo "\t$tmpY,$tmpX,$d,$origMoveDir\n";
            foreach ($movements as $move) {
                list($dy, $dx, $moveDir) = $move;
                if ($origMoveDir === false) {
                    $tmpOrigMoveDir = $moveDir;
                } else {
                    $tmpOrigMoveDir = $origMoveDir;
                }
                if (isset($planes[$y][$x][$tmpOrigMoveDir])) {
                    continue;
                }
                echo "\ttmpOrigMoveDir is $tmpOrigMoveDir\n";
                $newY = (4+$tmpY+$dy)%4;
                $newX = (4+$tmpX+$dx)%4;
                if ($newY == $y && $newX == $x) {
                    continue;
                }
                #echo "\t$moveDir\n";
                if (isset($planes[$newY][$newX])) {
                    if ($d == 1 || ($newY != $y && $newX != $x)) {
                        echo "\tfound $y,$x,$tmpOrigMoveDir: " . ($d+1) . " at $newY, $newX\n";
                        $planes[$y][$x][$tmpOrigMoveDir] = [ $newY, $newX, $d+1, $dirs[($tmpOrigMoveDir + ($d)*($dy+$dx)) % 4] ];
                        $found++;
                    }
                } else {
                    #echo "\tstoring $newY,$newX,".($d+1).",$origMoveDir in q\n";
                    $q[] = [$newY, $newX, $d+1, $tmpOrigMoveDir];
                }
            }
        }
    }
}
var_dump($planes);
die();


$x = min(array_keys($grid[0]));
$y = 0;
foreach ($chars as $charPos => $c) {
    if (is_numeric($c)) {
        $count .= $c;
        if (!isset($chars[$charPos+1]) || !is_numeric($chars[$charPos+1])) {
            echo "at num $count\n";
            $count = (int)$count;
            for ($i = 0; $i < $count; $i++) {
                list($dy,$dx) = $movements[$dir];
                $newY = $y+$dy;
                $newX = $x+$dx;
                if (!isset($grid[$newY][$newX])) {
                    list($newX, $newY, $turn) = moveP1($grid, $y, $x, $dirs[$dir]);
                }
                if ($grid[$newY][$newX] == SPACE) {
                    #echo "space";
                    $y = $newY;
                    $x = $newX;
                } else if ($grid[$newY][$newX] == WALL) {
                    break;
                    #echo "wall";
                    // do nothing
                } else {
                    throw new \Exception("wtf3");
                }
            }
            $count = "";

        }
    } else {
        $turn = $c;
        echo "at dir $turn\n";
        if ($turn == "L") {
            $dir = ($dir + 3) % 4;
        } elseif ($turn == "R") {
            $dir = ($dir + 1) % 4;
        } else {
            throw new \Exception("wtf");
        }
    }
    #Util::printGrid($grid, $y, $x, $movementChars[$dir]);
    #die();
}

$p1 = 1000*($y+1) + 4*($x+1) + $dir;

echo "P1: $p1\nP2: $p2\n";

function moveP2($grid, &$y, &$x, &$dir, $dirs, $movements, $count) {

            switch ($dirs[$dir]) {
                case EAST:
                    $newX = min(array_keys($grid[$y]));
                    break;
                case WEST:
                    $newX = max(array_keys($grid[$y]));
                    break;
                case SOUTH:
                    foreach ($grid as $k => $row) {
                        if (isset($row[$x])) {
                            $newY = $k;
                            break;
                        }
                    }
                    break;
                case NORTH:
                    foreach ($grid as $k => $row) {
                        if (isset($row[$x])) {
                            $newY = $k; // set to last el, because no break
                        }
                    }
                    break;
                default:
                    throw new \Exception("wtf2");
            }
}

function moveP1(&$grid, $y, $x, $dir) {
    switch ($dir) {
        case EAST:
            $newX = min(array_keys($grid[$y]));
            $newY = $y;
            break;
        case WEST:
            $newX = max(array_keys($grid[$y]));
            $newY = $y;
            break;
        case SOUTH:
            $newX = $x;
            foreach ($grid as $k => $row) {
                if (isset($row[$x])) {
                    $newY = $k;
                    break;
                }
            }
            break;
        case NORTH:
            $newX = $x;
            foreach ($grid as $k => $row) {
                if (isset($row[$x])) {
                    $newY = $k; // set to last el, because no break
                }
            }
            break;
        default:
            throw new \Exception("wtf2");
    }

    return [$newX, $newY, false];
}
